...
**Goal**: Run our checker on PyGoat (OWASP intentionally vulnerable Python/Django app), compare findings with pre-computed CodeQL results, and identify gaps in both tools.

**Context**:
- **PyGoat location**: `external_tools/pygoat/` (Django-based vulnerable web app with known security issues)
- **CodeQL location**: `external_tools/codeql/codeql/codeql` (CLI binary)
- **CodeQL results (PRE-COMPUTED - DO NOT RE-RUN)**: 
  - `results/pygoat_codeql/pygoat-codeql-results.csv` (raw CSV)
  - `results/pygoat_codeql/pygoat-codeql-results.sarif` (SARIF format)
  - `results/pygoat_codeql/CODEQL_RESULTS_SUMMARY.md` (human-readable summary)

**IMPORTANT**: CodeQL analysis has already been run on PyGoat. The results are stored in `results/pygoat_codeql/`. **Do NOT re-run CodeQL** - just read the existing results and compare with our checker's output.

### CodeQL Findings Summary (Pre-computed)

CodeQL found **31 issues** (18 errors, 13 warnings) in PyGoat:

| Category | CWE | Count | Type |
|----------|-----|-------|------|
| Code Injection | CWE-094 | 2 | eval() on user input |
| SQL Injection | CWE-089 | 2 | User input in SQL |
| Command Injection | CWE-078 | 2 | User input in shell commands |
| Unsafe Deserialization | CWE-502 | 3 | pickle.loads on user input |
| Clear-text Logging | CWE-312 | 5 | Passwords logged in plain text |
| Clear-text Storage | CWE-312 | 1 | Password stored unencrypted |
| SSRF | CWE-918 | 1 | User-controlled URL in request |
| Path Traversal | CWE-022 | 1 | User input in file path |
| XXE | CWE-611 | 1 | Unsafe XML parsing |
| XML Bomb | CWE-776 | 1 | Entity expansion attack |
| Weak Crypto | CWE-327 | 4 | MD5/SHA256 for passwords |
| Insecure Cookies | CWE-614 | 5 | Missing Secure/HttpOnly |
| Cookie Injection | CWE-020 | 2 | User input in cookie value |
| Flask Debug | CWE-215 | 1 | debug=True in production |

For full details, read `results/pygoat_codeql/CODEQL_RESULTS_SUMMARY.md`.

**Procedure**:

### Step 1: Read pre-computed CodeQL results

Read the existing results from `results/pygoat_codeql/CODEQL_RESULTS_SUMMARY.md` and `results/pygoat_codeql/pygoat-codeql-results.csv`.

### Step 2: Run our checker on PyGoat

```bash
cd /path/to/PythonFromScratch
venv/bin/python -m pyfromscratch.cli analyze external_tools/pygoat/ --recursive --output results/pygoat-our-results.json
```

### Step 3: Compare findings and write `checkers_lacks.md`
...
3. **DIFFERENT_SCOPE**: The bug type is out of scope for one tool (e.g., web-specific vs. general Python)
4. **FALSE_POSITIVE**: One tool incorrectly flagged something that isn't a bug

**You MUST write findings to `checkers_lacks.md`** in the repo root with this structure:
...
| Metric | Count |
|--------|-------|
| Total PyGoat Python files analyzed | X |
| CodeQL findings | X |
...
| CodeQL Query ID | Bug Category | Example in PyGoat | Priority to Add |
|-----------------|--------------|-------------------|-----------------|
| py/sql-injection | SQL Injection (taint tracking) | views.py:45 | HIGH |
| py/command-injection | OS Command Injection | utils.py:23 | HIGH |
| ... | ... | ... | ... |
...
| Our Bug Type | Description | Example in PyGoat |
|--------------|-------------|-------------------|
| ... | ... | ... |
...
- CodeQL excels at: [taint tracking, data flow, etc.]
- CodeQL misses: [semantic property bugs, barrier-style proofs, etc.]
- Our approach advantages: [formal guarantees, counterexample traces, etc.]
```

**Exit criteria**:
- CodeQL database created for PyGoat
- CodeQL security queries run successfully
- Our checker run on PyGoat
- `checkers_lacks.md` written with complete comparison
- `State.json.progress.evaluation.pygoat_codeql_comparison` updated with:
  - `completed: true`
  - `our_findings: [...]`
  - `codeql_findings: [...]`
  - `our_lacks: [...]` (bug types we need to add)
  - `codeql_lacks: [...]` (bug types CodeQL doesn't have)
- At least one follow-up action added to `queue.next_actions` to address a high-priority gap

**Why this phase matters**:
- Establishes baseline comparison with industry-standard tool
- Identifies concrete gaps to prioritize
- Validates that our semantic approach can find things taint-tracking misses
- Documents where we should NOT reinvent the wheel (e.g., if CodeQL's taint tracking is mature, focus on what they can't do)

**NOTE**: All CodeQL security bug types are now defined barrier-theoretically in `python-barrier-certificate-theory.md` §11. The comparison should confirm we can detect the same issues using our taint-tracking extensions to the symbolic VM.
