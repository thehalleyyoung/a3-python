...
class VerificationStatsHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        # Track Layer 0 papers (#21-25)
        elif '✓ [LAYER 0:' in msg or '[Paper #2' in msg:
            if 'LIKELY_INVARIANTS' in msg or 'Paper #21' in msg:
                self.stats['Layer 0: Paper #21 - Likely Invariants'] += 1
            elif 'SEPARATION_LOGIC' in msg or 'Paper #22' in msg:
                self.stats['Layer 0: Paper #22 - Separation Logic'] += 1
            elif 'REFINEMENT_TYPES' in msg or 'Paper #23' in msg:
                self.stats['Layer 0: Paper #23 - Refinement Types'] += 1
            elif 'INTERVAL_ANALYSIS' in msg or 'Paper #24' in msg:
                self.stats['Layer 0: Paper #24 - Interval Analysis'] += 1
            elif 'STOCHASTIC_BARRIERS' in msg or 'Paper #25' in msg:
                self.stats['Layer 0: Paper #25 - Stochastic Barriers'] += 1
            else:
                self.stats['Layer 0: Unknown Technique'] += 1
            self.total_verified_safe += 1
        
        # Track individual Papers #1-20
        elif 'Paper #1' in msg and 'Hybrid' in msg:
            self.stats['Paper #1: Hybrid Barriers'] += 1
            self.total_verified_safe += 1
        elif 'Paper #6' in msg and 'Structured' in msg:
            self.stats['Paper #6: Structured SOS'] += 1
            self.total_verified_safe += 1
        elif 'Paper #12' in msg and 'CEGAR' in msg:
            self.stats['Paper #12: CEGAR'] += 1
            self.total_verified_safe += 1
        elif 'Paper #16' in msg and 'CHC' in msg:
            self.stats['Paper #16: CHC Solving'] += 1
            self.total_verified_safe += 1
        elif 'Paper #17' in msg and 'ICE' in msg:
            self.stats['Paper #17: ICE Learning'] += 1
            self.total_verified_safe += 1
        elif 'Paper #19' in msg and 'SyGuS' in msg:
            self.stats['Paper #19: SyGuS'] += 1
            self.total_verified_safe += 1
        elif 'Paper #20' in msg and 'Assume' in msg:
            self.stats['Paper #20: Assume-Guarantee'] += 1
            self.total_verified_safe += 1
        
        # Track generic layers
        elif '✓ [LAYER 1' in msg or '[LAYER 2: SOS/SDP]' in msg:
            self.stats['Layers 1-2: Papers #1-8 (SOS/SDP)'] += 1
            self.total_verified_safe += 1
        elif '✓ [LAYER 3' in msg or '✓ [LAYER 4' in msg:
            self.stats['Layers 3-4: Papers #9-19 (Abstraction/Learning)'] += 1
            self.total_verified_safe += 1
        elif '✓ [LAYER 5' in msg:
            self.stats['Layer 5: Paper #20 (Compositional)'] += 1
            self.total_verified_safe += 1
...
def main():
    ...
    # Set up logging with DEBUG level for full visibility
    stats_handler = VerificationStatsHandler()
    stats_handler.setLevel(logging.DEBUG)  # Capture ALL debug messages
    ...
    root_logger.setLevel(logging.DEBUG)  # Enable DEBUG globally
    ...
    if cache_file.exists():
        ...
        # Cache the results (clear bytecode_instructions - not picklable)
        cache_file.parent.mkdir(exist_ok=True)
        print(f'  Saving to cache: {cache_file}')
        for summary in crash_summaries.values():
            summary.bytecode_instructions = []  # Clear before pickle
