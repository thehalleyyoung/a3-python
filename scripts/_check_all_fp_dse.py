#!/usr/bin/env python3
"""Check counterexample + DSE details for ALL FP cases."""
...
for cat, fname in fps:
    ...
    r = a.analyze_file(p)
    if r.verdict != "BUG":
        print(f"{cat:15s} {fname:50s}  verdict={r.verdict}")
        continue
    ce = r.counterexample or {}
    dse_val = ce.get("dse_validated", "NO_DSE")
    dse_status = ce.get("dse_result", {}).get("status", "NO_DSE")
    print(f"{cat:15s} {fname:50s}  bug={r.bug_type:20s} dse_val={str(dse_val):8s} dse_st={dse_status}")
